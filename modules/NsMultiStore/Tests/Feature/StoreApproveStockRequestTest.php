    ...
    public function testStockRequestApproval()
    {
        ...
        $response = $this->withSession($this->app['session']->all())
            ->json('post', 'api/stock-transfers/submit', [
                'name'          =>  __m('Test Stock Request - ', 'NsMultiStore').Str::random(10),
                'general'       =>  [
        ...
        $products->each(function ($product) use ($quantityToTransfer) {
            if ((float) ($product->newQuantity - $product->originalQuantity) != (float) $quantityToTransfer) {
                throw new Exception(sprintf(
                    __m('After transfect is approved, the stock has\'nt been updated : before => %s, after => %s, provided => %s, expected => %s', 'NsMultiStore'),
